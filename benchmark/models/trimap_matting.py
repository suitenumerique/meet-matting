...
class TrimapMattingWrapper(BaseModelWrapper):
    ...
    def __init__(
        ...
        self._pose_model: MediapipePoseWrapper | None = MediapipePoseWrapper()
    ...
    def predict(self, frame_bgr: np.ndarray) -> np.ndarray:
        ...
        if self._pose_model:
            limb_mask = self._pose_model.get_limb_mask(
                frame_work, thickness=self._limb_thickness, frame_rgb=frame_work_rgb
            )

        # ── Étape 3 : Trimap & GrabCut (Low-Res) ──
        binary_mask = (mask_semantic > 0.5).astype(np.uint8) * 255
        kernel_erode = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (self._erosion_size, self._erosion_size)
        )
        kernel_dilate = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (self._dilation_size, self._dilation_size)
        )
        ...
        try:
            cv2.grabCut(  # type: ignore[call-overload]  # rect peut être None avec GC_INIT_WITH_MASK
                frame_work,
                grabcut_mask,
                None,
                bgd_model,
        ...
        if max(h_orig, w_orig) > limit_size:
            ...
            guide = cv2.cvtColor(
                cv2.resize(frame_bgr, (w_opt, h_opt), interpolation=cv2.INTER_AREA),
                cv2.COLOR_BGR2GRAY,
            )
            src = cv2.resize(low_res_refined, (w_opt, h_opt), interpolation=cv2.INTER_LINEAR)
            mask_final = cv2.ximgproc.guidedFilter(
                guide, src, self._refine_radius, self._refine_eps
            )
            mask_final = cv2.resize(mask_final, (w_orig, h_orig), interpolation=cv2.INTER_LINEAR)
        else:
            mask_refined = cv2.resize(
                low_res_refined, (w_orig, h_orig), interpolation=cv2.INTER_LINEAR
            )
            guide = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)
            mask_final = cv2.ximgproc.guidedFilter(
                guide, mask_refined, self._refine_radius, self._refine_eps
            )
        ...
        if self._prev_mask is not None and self._prev_mask.shape == mask_final.shape:
            mask_final = cv2.addWeighted(
                mask_final, self._ema_alpha, self._prev_mask, 1.0 - self._ema_alpha, 0
            )
