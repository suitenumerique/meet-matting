...
This wrapper uses ONNX Runtime. The ONNX model must be provided
(converted from the official repo or a TorchScript export).

Model: https://github.com/microsoft/Cream/tree/main/EfficientViT
"""
...
class EfficientViTWrapper(BaseModelWrapper):
    ...
    def load(self) -> None:
        ...
        if not self._model_path.exists():
            ...
            logger.info("EfficientViT: downloading from %s", self._MODEL_URL)
            try:
                urllib.request.urlretrieve(self._MODEL_URL, str(self._model_path))
            except Exception as e:
                logger.error("EfficientViT: download error: %s", e)
                return
