...
_DEFAULT_MODEL_PATH = Path(__file__).parent.parent / "weights" / "efficientvit_seg.onnx"
...
class EfficientViTWrapper(BaseModelWrapper):
    ...
    def load(self) -> None:
        ...
        try:
            ...
            if "INVALID_PROTOBUF" in str(e) or "Protobuf parsing failed" in str(e):
                logger.warning(
                    "EfficientViT: Le fichier semble corrompu. Suppression de %s pour un futur téléchargement.",
                    self._model_path,
                )
    ...
    def predict_batch(self, frames_bgr: list[np.ndarray]) -> list[np.ndarray]:
        ...
        logger.info(
            "EfficientViT: Metadata entrée - Shape: %s, Name: %s", input_shape, input_meta.name
        )
        ...
        logger.info(
            "EfficientViT: Layout détecté: %s, Canaux attendus: %d", layout, expected_channels
        )
        ...
        try:
            ...
            if "channels C" in error_msg and "not equal to kernel channels" in error_msg:
                logger.info(
                    "EfficientViT: Détection d'un modèle à 4 canaux. Transformation en cours..."
                )
                ...
                logger.debug(
                    "EfficientViT: Batching non supporté par le modèle, repli sur itération."
                )
                logits_batch = []
        ...
        for i in range(batch_size):
            ...
            if logits.ndim == 3:  # Format (C, H, W) - Segmentation
                ...
                if num_classes == 1:
                    ...
                    idx = (
                        ...
                        if num_classes > self.PERSON_CLASS_INDEX
                        else (1 if num_classes > 1 else 0)
                    )
                    ...
                    mask = probs[idx]
            elif logits.ndim == 1:  # Format (C,) - Erreur, c'est de la classification
                logger.error(
                    "EfficientViT: Le modèle chargé est un modèle de classification, pas de segmentation."
                )
