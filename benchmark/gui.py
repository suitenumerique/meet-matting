...
class BenchmarkGUI:
    ...
    def _create_widgets(self):
        ...
        # ── Header ──
        ttk.Label(main_frame, text="Benchmark Video Matting", style="Header.TLabel").pack(pady=(0, 20))
        ...
        ttk.Checkbutton(video_frame, text="Sélection aléatoire (Shuffle)", variable=self.shuffle_var).pack(anchor="w", pady=(10,0))
        ...
        ttk.Button(btn_frame, text="Tous", command=self._select_all_models, width=10).pack(side="left", padx=2)
        ttk.Button(btn_frame, text="Aucun", command=self._select_no_models, width=10).pack(side="left", padx=2)
    ...
    def _run_benchmark_task(self, selected_keys, num_videos):
        try:
            ...
            self.root.after(0, lambda: self._on_complete("Erreur", f"Une erreur est survenue : {str(e)}", is_error=True))
