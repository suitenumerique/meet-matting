...
@models.register
class MediapipeMulticlass(MattingModel):
    name = "mediapipe_multiclass"
    description = "MediaPipe Selfie Multiclass — 5-class segmenter (hair, skin…), 256×256 TFLite."
    ...
    def infer(self, frame: np.ndarray) -> np.ndarray:
        ...
        if result and result.confidence_masks and len(result.confidence_masks) > 1:
            # Index 0 is background — invert to get foreground
            mask_small = 1.0 - result.confidence_masks[0].numpy_view()
        elif result and result.confidence_masks:
